...
@section('breadcrumb')
    <li>
        <div class="flex items-center">
            ...
            <span class="ml-1 text-sm font-medium text-gray-500">Classes</span>
        </div>
    </li>
@endsection

@section('page-title', 'Class Management')
@section('page-description', 'Manage graduation classes and their members')

@section('page-actions')
    <a href="{{ route('admin.classes.create') }}" class="bg-fcs-primary hover:bg-fcs-light-blue text-white px-6 py-3 rounded-lg font-semibold transition shadow-md">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($classes as $class)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900">{{ $class->graduation_year }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900">{{ $class->members_count }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900">{{ $class->activities_count }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <form action="{{ route('admin.classes.toggle-status', $class) }}" method="POST" class="inline">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $class->is_active ? 'bg-green-100 text-green-800 hover:bg-green-200' : 'bg-red-100 text-red-800 hover:bg-red-200' }} transition">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex space-x-2">
                                    <a href="{{ route('admin.classes.show', $class) }}" class="text-fcs-blue hover:text-fcs-light-blue">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    <a href="{{ route('admin.classes.edit', $class) }}" class="text-yellow-600 hover:text-yellow-700">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <a href="{{ route('admin.classes.members', $class) }}" class="text-green-600 hover:text-green-700">
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <a href="{{ route('admin.classes.create') }}" class="bg-fcs-primary hover:bg-fcs-light-blue text-white px-4 py-2 rounded-lg transition">
                                        Create Class
                                    </a>
                                </div>
                            </td>
